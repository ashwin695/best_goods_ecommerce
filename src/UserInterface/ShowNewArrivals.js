...
export default function ShowNewArrivals(props){
    ...
    const showNewArrivals=()=>{
        return newArrivalList.map((item)=>{
    ...
    useEffect(function(){
        window.scrollTo(0,0)
        fetchAllNewArrivals()
        fetchAllProductsexceptnewarrival()
    },[])
    ...
    const fetchAllProductsexceptnewarrival=async()=>{
        var result=await getData("finalproduct/displayallfinalproductsexceptnewarrival")
